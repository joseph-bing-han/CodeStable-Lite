...
计划确定前可以通过结构化问题澄清目标、边界与不可逆授权。计划写入 Task 后进入无人值守：Agent 不再反复询问普通实现路线，而是按契约一致性、风险、可逆性、证据强度和总成本自动选择推荐方向，处理失败并持续到全部计划、验证和 Task 归档完成。
...
- Talk 在计划确定前不落盘；Vision 的目标内容、Epic 关闭和危险操作也都在计划确定前保留人为授权。Task 创建后，普通执行分叉不再反复确认。
